...
public class Auth implements AuthInterface {
    ...
    public void register(UserDao userDao, Entity user) {
        if(userInformationValidation.validateAll(user) == true){
            // Kullanici bilgileri kuralla uygun girilmis mi
            if(authCheck.checkForRegister((User) user) == false){
                ...
                if(confirmationEmail.verifyEmail() == true){
                    ...
                    userInformations.addInformation((User) user);
            ...
            userInformationValidation.showUnvalidateInformations((User) user);
        }
    }
}
